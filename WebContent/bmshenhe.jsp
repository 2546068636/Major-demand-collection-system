...
<body style="margin-top: 15px; margin-left: 33px;">
	...
	<form class="layui-form" method="post"
		action="xuqiuservlet?method=bmagreeornot">
		<input type="hidden" value="${param.id }" name="id"> <input
			type="hidden" value="${param.tp }" name="tp"> <input
			type="hidden" value="${param.glcs }" name="glcs">
		<div class="layui-form-item" pane="">
			<label class="layui-form-label"
				style="font-size: 12px; font-style: SimHei">是否同意</label>
			<div class="layui-input-block">
				<input type="radio" lay-filter="isornot" name="isornotbm" value="同意" />同意
				<!-- 单选按钮的name要一样 -->
				<input type="radio" lay-filter="isornot" name="isornotbm"
					value="不同意" />不同意
				<!-- selected 和它的区别是什么？都是选择 -->
			</div>
		</div>
		...
		<div class="layui-input-block ">
			...
			<textarea name="bmcomment" style="display: none"
